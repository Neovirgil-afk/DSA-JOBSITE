(function () {
    ...
    const state = { recommendations: [], catalog: [], activeSkill: '', activeLesson: 0 };
    ...
    function renderLesson(lesson, resources) {
        ...
        content.innerHTML = '<div class="learning-content-top"><div><span class="learning-eyebrow">BEGINNER LESSON</span><h2>' + escapeHTML(lesson.skill) + '</h2><p>' + escapeHTML(lesson.description) + '</p></div><span class="learning-badge">Beginner</span></div>' +
            ...
            '</div></section>' +
            '<div class="learning-actions"><button type="button" class="learning-action-button" id="previousLesson" ' + (state.activeLesson === 0 ? 'disabled' : '') + '>← Previous</button><span class="learning-action-copy">' + (state.activeLesson === total - 1 ? 'Foundation complete — assessment comes next.' : 'Read the concept, then continue.') + '</span><button type="button" class="learning-action-button primary" id="nextLesson">' + (state.activeLesson === total - 1 ? 'Finish Lesson' : 'Next Lesson →') + '</button></div>';

        $('#previousLesson')?.addEventListener('click', () => { if (state.activeLesson > 0) { state.activeLesson--; renderLesson(lesson, resources); } });
        $('#nextLesson')?.addEventListener('click', () => { if (state.activeLesson < total - 1) { state.activeLesson++; renderLesson(lesson, resources); } else { window.location.href = '/profile.html'; } });
    }
